...
export const useStore = create<AppState>((set, get) => ({
  ...
  loadLesson: async (courseId, lessonId) => {
    ...
    try {
      ...
      const mockLessons = {
        1: [
          { id: 1, course_id: 1, title: 'Python环境搭建', content: 'Python环境搭建是学习Python的第一步。在本课时中，我们将详细学习：\n\n1. Python的安装方法：\n   - 从官方网站下载并安装Python\n   - 配置环境变量\n   - 验证安装是否成功\n\n2. 开发环境选择：\n   - 命令行终端\n   - IDLE（Python自带编辑器）\n   - Visual Studio Code\n   - PyCharm\n\n3. Jupyter Notebook的安装和使用：\n   - 安装Jupyter Notebook\n   - 创建和运行Notebook\n   - Markdown语法基础\n\n4. 第一个Python程序：\n   - 输出"Hello, World!"\n   - 基本的交互式命令\n\n通过本课时的学习，您将能够搭建一个完整的Python开发环境，并开始编写和运行Python代码。', pitfalls: '1. 环境变量配置错误：安装Python后，忘记将Python添加到系统环境变量中，导致无法在命令行中直接运行Python。\n2. 版本选择错误：下载了不兼容的Python版本，导致后续库安装失败。\n3. 路径包含空格：Python安装路径包含空格，导致某些命令无法正常执行。\n4. Jupyter Notebook启动失败：缺少必要的依赖包，导致Jupyter Notebook无法启动。\n5. 权限问题：安装过程中没有以管理员身份运行，导致安装不完整。', order: 1, duration: '45分钟' },
          { id: 2, course_id: 1, title: 'Python基础语法', content: 'Python基础语法是Python编程的基础。在本课时中，我们将详细学习：\n\n1. 基本数据类型：\n   - 整数、浮点数、字符串\n   - 布尔值\n   - 列表、元组、字典、集合\n\n2. 变量和赋值：\n   - 变量命名规则\n   - 多重赋值\n   - 变量类型转换\n\n3. 运算符：\n   - 算术运算符\n   - 比较运算符\n   - 逻辑运算符\n   - 成员运算符\n\n4. 控制流：\n   - if-elif-else条件语句\n   - for循环\n   - while循环\n   - break和continue语句\n\n5. 输入输出：\n   - print()函数\n   - input()函数\n   - 格式化输出\n\n通过本课时的学习，您将掌握Python的基本语法，能够编写简单的Python程序。', pitfalls: '1. 缩进错误：Python使用缩进来表示代码块，缩进不一致会导致语法错误。\n2. 变量命名错误：使用了Python关键字作为变量名，或者变量名不符合命名规则。\n3. 类型错误：在操作中混合使用了不同类型的数据，如字符串和数字相加。\n4. 循环条件错误：while循环的条件设置不当，导致无限循环。\n5. 字符串引号混用：在字符串中混用单引号和双引号，导致语法错误。\n6. 冒号遗漏：在if、for、while等语句后忘记添加冒号。', order: 2, duration: '60分钟' },
          { id: 3, course_id: 1, title: 'Python函数和模块', content: 'Python函数和模块是Python编程的重要组成部分。在本课时中，我们将详细学习：\n\n1. 函数定义和调用：\n   - def关键字\n   - 函数参数\n   - 默认参数\n   - 可变参数\n   - 函数返回值\n\n2. 作用域：\n   - 局部变量\n   - 全局变量\n   - nonlocal关键字\n\n3. 模块：\n   - 模块导入\n   - __name__变量\n   - 包的概念\n\n4. 常用内置函数：\n   - len()、max()、min()\n   - range()、list()、dict()\n   - zip()、enumerate()\n\n通过本课时的学习，您将能够定义和使用函数，以及创建和使用模块，提高代码的复用性和可维护性。', pitfalls: '1. 函数参数顺序错误：调用函数时参数顺序与定义不一致。\n2. 作用域混淆：在函数内部修改全局变量时忘记使用global关键字。\n3. 模块导入错误：导入模块时路径错误，或者模块名称拼写错误。\n4. 循环导入：两个模块相互导入，导致导入错误。\n5. 函数返回值缺失：函数应该返回值但没有return语句。\n6. 默认参数陷阱：使用可变对象作为默认参数，导致多次调用时共享状态。', order: 3, duration: '60分钟' },
          { id: 4, course_id: 1, title: 'NumPy基础', content: 'NumPy是Python中用于科学计算的重要库。在本课时中，我们将详细学习：\n\n1. NumPy简介：\n   - NumPy的安装\n   - NumPy的核心概念\n\n2. 数组创建：\n   - 从列表创建\n   - 内置函数创建（zeros、ones、arange等）\n   - 随机数生成\n\n3. 数组属性：\n   - shape、dtype、size\n   - ndim\n\n4. 数组索引和切片：\n   - 基本索引\n   - 切片操作\n   - 布尔索引\n   - 花式索引\n\n5. 数组操作：\n   - 基本运算\n   - 广播机制\n   - 常用数学函数\n\n通过本课时的学习，您将掌握NumPy的基本使用方法，为后续的数据分析打下基础。', pitfalls: '1. 数组形状错误：在运算时数组形状不匹配，导致广播失败。\n2. 索引越界：访问数组时使用了超出范围的索引。\n3. 数据类型错误：数组数据类型与操作不兼容，如整数数组进行除法运算。\n4. 内存消耗过大：创建了过大的数组，导致内存不足。\n5. 视图与副本混淆：修改数组视图时意外修改了原始数组。\n6. 广播规则不理解：不了解NumPy的广播规则，导致运算结果不符合预期。', order: 4, duration: '60分钟' },
          { id: 5, course_id: 1, title: 'Pandas基础', content: 'Pandas是Python中用于数据处理和分析的重要库。在本课时中，我们将详细学习：\n\n1. Pandas简介：\n   - Pandas的安装\n   - Pandas的核心数据结构\n\n2. Series：\n   - Series的创建\n   - Series的索引和切片\n   - Series的基本操作\n\n3. DataFrame：\n   - DataFrame的创建\n   - DataFrame的索引和切片\n   - DataFrame的基本操作\n   - 数据类型转换\n\n4. 数据读取和写入：\n   - 读取CSV文件\n   - 读取Excel文件\n   - 写入数据\n\n5. 数据清洗：\n   - 处理缺失值\n   - 处理重复值\n   - 数据排序\n\n通过本课时的学习，您将掌握Pandas的基本使用方法，能够处理和分析实际的数据集。', pitfalls: '1. 索引错误：使用了不存在的列名或行索引。\n2. 数据类型转换错误：尝试将不兼容的数据类型转换为其他类型。\n3. 缺失值处理不当：在处理缺失值时选择了不合适的方法，导致数据失真。\n4. 内存使用过高：处理大型数据集时内存不足。\n5. 链式赋值警告：使用链式操作修改DataFrame时出现警告。\n6. 索引对齐问题：在操作两个DataFrame时，索引不一致导致结果不符合预期。\n7. 文件路径错误：读取文件时路径错误，导致文件找不到。', order: 5, duration: '60分钟' },
          { id: 6, course_id: 1, title: '数据可视化基础', content: '数据可视化是数据分析的重要组成部分。在本课时中，我们将详细学习：\n\n1. Matplotlib简介：\n   - Matplotlib的安装\n   - 基本概念\n\n2. 基本图表类型：\n   - 折线图\n   - 柱状图\n   - 散点图\n   - 饼图\n   - 直方图\n\n3. 图表定制：\n   - 标题和标签\n   - 图例\n   - 颜色和样式\n   - 网格和背景\n\n4. 多子图：\n   - subplot()函数\n   - subplots()函数\n\n5. 中文显示问题：\n   - 字体设置\n   - 负号显示\n\n通过本课时的学习，您将能够创建各种类型的图表，直观地展示数据。', pitfalls: '1. 中文显示乱码：没有正确设置中文字体，导致图表中的中文显示为乱码。\n2. 图表大小不合适：创建的图表太小或太大，影响可读性。\n3. 坐标轴标签重叠：坐标轴标签太多，导致重叠无法看清。\n4. 颜色选择不当：使用了不利于色盲人士阅读的颜色组合。\n5. 图表类型选择错误：选择了不适合数据类型的图表类型。\n6. 图例位置不当：图例覆盖了图表中的重要数据。\n7. 内存泄漏：创建大量图表时没有正确释放资源，导致内存泄漏。', order: 6, duration: '60分钟' },
          { id: 7, course_id: 1, title: '商业数据分析案例', content: '商业数据分析案例是将Python应用于实际商业场景的重要实践。在本课时中，我们将通过一个完整的商业数据分析案例，详细学习：\n\n1. 案例背景：\n   - 电商销售数据\n   - 分析目标\n\n2. 数据收集和导入：\n   - 数据格式\n   - 使用Pandas读取数据\n\n3. 数据清洗和预处理：\n   - 处理缺失值\n   - 数据类型转换\n   - 数据筛选和过滤\n\n4. 数据探索和分析：\n   - 基本统计分析\n   - 按产品类别分析\n   - 按时间趋势分析\n   - 客户购买行为分析\n\n5. 数据可视化：\n   - 销售趋势图\n   - 产品类别分布图\n   - 客户购买行为分析图\n\n6. 分析结论和建议：\n   - 关键发现\n   - 业务建议\n\n通过本课时的学习，您将能够将所学知识应用到实际的商业数据分析场景中。', pitfalls: '1. 数据质量问题：使用了质量差的数据，导致分析结果不可靠。\n2. 分析目标不明确：没有明确的分析目标，导致分析方向混乱。\n3. 过度分析：分析了与业务无关的数据，浪费时间和资源。\n4. 数据可视化不当：创建了过于复杂或不清晰的图表，影响信息传达。\n5. 结论不严谨：基于有限的数据得出过于绝对的结论。\n6. 忽略数据偏差：没有考虑数据采集过程中的偏差，导致分析结果失真。\n7. 缺乏业务知识：不了解业务背景，导致分析结果与实际业务需求不符。', order: 7, duration: '90分钟' },
          { id: 8, course_id: 1, title: '课程总结与项目实践', content: '课程总结与项目实践是对本课程内容的回顾和应用。在本课时中，我们将：\n\n1. 课程内容总结：\n   - Python基础语法\n   - NumPy和Pandas库\n   - 数据可视化\n   - 商业数据分析\n\n2. 项目实践：\n   - 项目目标：分析销售数据，识别销售趋势和客户行为\n   - 数据准备：使用提供的销售数据集\n   - 分析步骤：\n     * 数据导入和清洗\n     * 数据探索和分析\n     * 数据可视化\n     * 生成分析报告\n\n3. 学习资源推荐：\n   - 书籍\n   - 在线课程\n   - 文档和社区\n\n4. 后续学习方向：\n   - 高级Python编程\n   - 机器学习基础\n   - 深度学习入门\n   - 大数据处理\n\n通过本课时的学习，您将巩固所学知识，并完成一个完整的商业数据分析项目。', pitfalls: '1. 项目范围过大：选择了过于复杂的项目，导致无法在有限时间内完成。\n2. 数据准备不足：没有充分准备数据，导致分析过程中遇到大量数据问题。\n3. 技术选择不当：选择了不适合项目需求的技术工具。\n4. 代码组织混乱：代码结构不清晰，难以维护和理解。\n5. 分析深度不够：只进行了表面分析，没有深入挖掘数据价值。\n6. 报告表达不清：分析报告结构混乱，重点不突出。\n7. 缺乏反思和改进：没有对项目过程进行反思，无法从中学习和改进。', order: 8, duration: '90分钟' }
        ],
        2: [
